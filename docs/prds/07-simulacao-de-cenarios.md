...
A simulação usa exatamente a mesma lógica de cálculo do resultado real, só que sobre uma configuração hipotética. Não é uma segunda calculadora.

## Histórias de usuário

- Como **membro da coordenação**, quero simular a substituição de um modelo de equipamento por outro, para estimar o ganho antes de pedir orçamento.
...
- **Dado** que tenho um resultado real calculado, **quando** crio um cenário a partir dele, **então** o cenário começa com uma cópia exata da configuração atual.
- **Dado** que estou em um cenário, **quando** substituo um modelo de equipamento por outro, **então** o resultado é recalculado com as características do novo modelo.
- **Dado** que estou em um cenário, **quando** altero o sistema operacional, **então** o novo consumo é aplicado — e, se não houver medição para aquela combinação de modelo e sistema, a plataforma avisa que o resultado depende de uma estimativa.
...
- **Dado** que vejo um cenário calculado, **então** vejo lado a lado o valor real, o valor simulado, a diferença absoluta e a diferença percentual.
...
- Simulação de emissões de fabricação e descarte dos equipamentos substituídos — pertence ao escopo 3.
...
- Em aberto: o cenário deve permitir alterar o período letivo, ou apenas equipamento, sistema operacional e ocupação?
